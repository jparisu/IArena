...
from IArena.utils.square_map.SquareMap import Coordinate, Direction, SquareMap
...
class GoldMinePosition(IPosition):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return self.__current_position == other.__current_position and self.__digged_tiles == other.__digged_tiles
    ...
    def get_cost_from_direction(self, direction: Direction) -> CostType:
        """
    ...
    def _plot_step(self, axis, start: Coordinate, target: Coordinate, smap: GoldMineSquareMap, **kwargs):
        """
...
class GoldMineGameRules(IGameRules):
    ...
    def __init__(
        ...
        self.__heuristic_map: GoldMineSquareMap = (
            heuristic_map if heuristic_map is not None else GoldMineSquareMap.zeros_like(self.__map)
        )

        # Check map
        self.__map.check(allow_zero=False, skip_coordinate_validation=[self.__start])
    ...
    @override
    def next_position(self, movement: GoldMineMovement, position: GoldMinePosition) -> GoldMinePosition:
    ...
    @override
    def possible_movements(self, position: GoldMinePosition) -> Iterator[GoldMineMovement]:
    ...
    def _get_valid_directions(self, coordinate: Coordinate) -> Iterator[Direction]:
        ...
        This method is meant only to be used from GoldMinePosition.
        """
        yield from self.__map.possible_directions(coordinate)

    def _get_cost(self, coordinate: Coordinate) -> CostType:
        """
    ...
    def plot_step(self, axis, position: GoldMinePosition, **kwargs):
        """Plot the current game step on the given matplotlib axis."""
        position._plot_step(axis=axis, start=self.__start, target=self.__target, smap=self.__map, **kwargs)
...
class GoldMineGenerator(IRulesGenerator):
    def generate(self, configuration: RulesGeneratorConfiguration) -> GoldMineGameRules:
        ...
        if starting_position.x < 0 or starting_position.y < 0:
            starting_position = Coordinate(x=rng.randint(map_height), y=rng.randint(map_width))

        # GENERATE TARGET POSITION
        while target_position is None or target_position == starting_position:
            target_position = Coordinate(x=rng.randint(map_height), y=rng.randint(map_width))
